...
def parse_args(args):
    parser = argparse.ArgumentParser(description="M2SA Model Training")
    ...
    parser.add_argument("--vision_pretrained", default="./vision_pretrained/sam_vit_h_4b8939.pth", type=str)
...
def main(args):
    ...
    if not args.eval_only:
        model.get_model().initialize_M2SA_modules(model.get_model().config)
    ...
    train_dataset = HybridDataset(
        ...
        * args.grad_accumulation_steps # grad_accumulation_steps: 10
        * args.steps_per_epoch # steps_per_epoch: 500
        * world_size, # GPU device count
        precision=args.precision, 
        image_size=args.image_size, # 1024
        num_classes_per_sample=args.num_classes_per_sample, # num_classes_per_sample: 3
        dataset=args.dataset,
        sample_rate=[float(x) for x in args.sample_rates.split(",")], # 9, 3, 3, 1
